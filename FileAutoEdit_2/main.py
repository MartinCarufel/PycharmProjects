...
class Ask_compte(Tk):
    def __init__(self):
        ...
        self.choix.set(value='n')
        self.geometry('400x200')
        self.frame = Frame(self)
        self.label = Label(text="Un petite fenetre")
        self.closeButton = Button(self, text='Close', command=self.quit)
        self.frame.pack()
        self.label.pack()
        self.rb1 = Radiobutton(text='Martin', variable=self.choix, value='m')
        self.rb2 = Radiobutton(text='Anne-Marie', variable=self.choix, value='a')
        self.rb3 = Radiobutton(text='Conjoint', variable=self.choix, value='c')
        self.rb1.pack()
        self.rb2.pack()
        self.rb3.pack()
...
def compte_pour(content):
    ...
    if not found:
        ...
        print(content)
        if content[0] == 'VISA 4540330755072014':
            print("Carte: {} - Martin - {} - {} - {} ".format(content[0], content[1], content[2],
                                                              content[4]))
        ...
        elif content[0] == 'VISA 4540330755072022':
            print("Carte: {} - Anne-Marie - {} - {} - {} ".format(content[0], content[1], content[2],
                                                                  content[4]))
            popup = Ask_compte()
            popup.mainloop()
            # choice = input("Choix: (m, a, c ) ? ")
